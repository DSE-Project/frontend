    ...
    <>
      <div className={`bg-white shadow-lg h-full fixed left-0 top-16 bottom-0 z-40 transition-all duration-300 ${
        ...
        <button
          ...
          className="absolute -right-3 top-8 bg-white shadow-md rounded-full p-1.5 hover:bg-gray-50 transition-colors duration-200 z-50"
              ...
              return (
                <li key={item.path} className="relative group">
                  {isAccessible ? (
                    <Link
                      to={item.path}
                      className={`flex items-center ${isCollapsed ? 'px-4 justify-center' : 'px-6'} py-3 text-sm font-medium transition-colors duration-200 ${
                        isActive
                    ...
                    >
                      <span className={`${isCollapsed ? '' : 'mr-3'} ${isActive ? 'text-blue-700' : 'text-gray-400'}`}>
                        {item.icon}
                      </span>
                    ...
                    <div
                      className={`flex items-center ${isCollapsed ? 'px-4 justify-center' : 'px-6'} py-3 text-sm font-medium cursor-not-allowed text-gray-400`}
                      onClick={(e) => handleItemClick(e, item)}
                      title={isCollapsed ? `${item.name} - Please login to access` : ''}
                    >
                      <span className={`${isCollapsed ? '' : 'mr-3'} text-gray-300`}>
                        {item.icon}
                      </span>
                      {!isCollapsed && (
                        <>
